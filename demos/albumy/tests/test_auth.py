...
class AuthTestCase(BaseTestCase):
  ...
  def test_login_blocked_user(self):
    response = self.login(email='blocked@helloflask.com', password='12345678')
    data = response.get_data(as_text=True)
  ...
  def test_reigster_account(self):
  ...
  def test_bad_confirm_token(self):
    self.login(email='unconfirmed@helloflask.com', password='12345678')
    response = self.client.get(
      url_for('auth.confirm', token='bad token'),
      follow_redirects=True
    )
    data = response.get_data(as_text=True)
  ...
  def test_reset_password(self):
    response = self.client.post(
      ...
      data=dict(email='normal@helloflask.com'),
      follow_redirects=True
    )
    data = response.get_data(as_text=True)
    self.assertIn('Password reset email sent, check you inbox.', data)
    ...
    # bad token
    response = self.client.post(
      url_for('auth.reset_password', token='bad token'),
      data=dict(
        email='normal@helloflask.com',
        password='new-password',
        password2='new-password',
